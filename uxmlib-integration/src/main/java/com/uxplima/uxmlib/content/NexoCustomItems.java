...
/**
 * CustomItems over Nexo.
 *
 * <p>Nexo is Oraxen's successor and keeps the same model under different names. A server migrating from one to the other runs both for a while, which is exactly what the composite is for.
 *
 * <p>Reflective, like every provider in this package: not one Nexo type is named in a signature or a
 ...
 */
public final class NexoCustomItems implements CustomItems {

    /** The Bukkit plugin name, which is the whole of the present guard. */
    static final String PLUGIN = "Nexo";

    /** The namespace an id of this vendor's carries, so two vendors' ids can never be confused. */
    static final String NAMESPACE = "nexo";

    private static final String API = "com.nexomc.nexo.api.NexoItems";

    private final Server server;

    public NexoCustomItems(Server server) {
    ...
    public Optional<String> idOf(ItemStack stack) {
        ...
        return Reflected.method(API, "idFromItem", "org.bukkit.inventory.ItemStack")
                .flatMap(method -> Reflected.callForString(method, null, stack))
                .map(NexoCustomItems::namespaced);
    }
    ...
    public Optional<String> idOfBlock(Block block) {
        ...
        return Reflected.method(API, "idFromItem", "org.bukkit.block.Block")
    ...
    public Optional<ItemStack> itemOf(String id) {
        ...
        Method builder = Reflected.method(API, "itemFromId", "java.lang.String").orElse(null);
        if (builder == null) {
            return Optional.empty();
        }
        return Reflected.call(builder, null, plain).map(NexoCustomItems::asItem).filter(Objects::nonNull);
    }
