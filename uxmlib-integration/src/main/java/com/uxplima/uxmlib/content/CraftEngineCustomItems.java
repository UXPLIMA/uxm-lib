...
/**
 * CustomItems over CraftEngine.
 *
 * <p>CraftEngine prices a block and an item apart, which is the shape this seam wants, so the two questions map onto two calls rather than onto one.
 *
 * <p>Reflective, like every provider in this package: not one CraftEngine type is named in a signature or a
 ...
 */
public final class CraftEngineCustomItems implements CustomItems {

    /** The Bukkit plugin name, which is the whole of the present guard. */
    static final String PLUGIN = "CraftEngine";

    /** The namespace an id of this vendor's carries, so two vendors' ids can never be confused. */
    static final String NAMESPACE = "craftengine";

    private static final String API = "net.momirealms.craftengine.bukkit.api.CraftEngineItems";

    private final Server server;

    public CraftEngineCustomItems(Server server) {
    ...
    public Optional<String> idOf(ItemStack stack) {
        ...
        return Reflected.method(API, "getCustomItemId", "org.bukkit.inventory.ItemStack")
                .flatMap(method -> Reflected.callForString(method, null, stack))
                .map(CraftEngineCustomItems::namespaced);
    }
    ...
    public Optional<String> idOfBlock(Block block) {
        ...
        return Reflected.method(API, "getCustomBlockId", "org.bukkit.block.Block")
    ...
    public Optional<ItemStack> itemOf(String id) {
        ...
        Method builder = Reflected.method(API, "byId", "java.lang.String").orElse(null);
        if (builder == null) {
            return Optional.empty();
        }
        return Reflected.call(builder, null, plain)
                .map(CraftEngineCustomItems::asItem)
